...
hf_data_path = "../data/unadj_Cnts_RBCfilt_ensembl.csv"
...
write.csv(HF_LM_data, "../data/HF_LM_genes_counts.csv")
...
```{r}
# use for subsetting if needed
col_ids = keep_perts$inst_id[1:100]
level2_gctx_in <- parse_gctx(lincs_data_path) %>% 
  mat() %>%
  as.data.frame()
...
```{r concat HF data}
HF_LM_data_merge <- HF_LM_data[, -which(names(HF_LM_data) %in% c("external_gene_name", "X", "Row.names.y", "pr_gene_title", "pr_is_lm", "pr_is_bing"))]
level2_hf_genes <- merge(level2_hf_genes, HF_LM_data_merge, by.x = 'row.names', by.y = "pr_gene_id") %>% column_to_rownames("Row.names")

```

```{r writeOut}
rm(level2_gctx_in, level2_hf_genes)

# Port to a gctx for faster loading downstream
level2_hf_gctx_out <- new("GCT", mat=as.matrix(level2_pEFmrg_hf_genes))

# Save
write_gctx(level2_hf_gctx_out, "../data/shared_landmark_counts_vecs.gctx", compression_level = 8)
```
